    ...
    def capture(self):
        ...
        for j in range(0,30+self.world_size-1):
            ...
            if "jpg" in f :
                ...
                comm.send(image,dest=request,tag=1)           
                counter+=1
        end_time = time.time()
        print (f"inference on {counter} images took {end_time-start_time} on {size-1} workers ")
        print (f"{counter/(end_time-start_time)} fps on {size-1} workers mode 2")
        exit()
def run ():   
    #print (rank,"rank")
    if rank==1:
        ...
        while True:
            ...
            #print (image,rank ,end=" , ")
            if type(f)==int:
                print("finished") 
                break
            
            inp=f.reshape(1,224,224,3).astype("float32")/255
